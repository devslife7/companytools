...
import { calculateBatch, formatMLValue, formatNumber, LITER_TO_ML } from '../lib/calculations'
...
export function BatchingInstructionsModal({ batch, onClose }: BatchingInstructionsModalProps) {
    ...
    const { selectedCocktail, servings } = batch
                ...
                <div className="p-6 overflow-y-auto">
                    ...
                    <div className="flex flex-wrap gap-4 mb-6 text-sm">
                        ...
                        <div className="bg-white rounded-lg p-3 px-4 border border-gray-200 shadow-sm flex-1 min-w-[140px]">
                            ...
                            <span className="text-lg font-medium text-gray-900">{selectedCocktail.glassType || 'N/A'}</span>
                        </div>
                        <div className="bg-white rounded-lg p-3 px-4 border border-gray-200 shadow-sm flex-[2] min-w-[200px]">
                            ...
                            <span className="text-lg font-medium text-gray-900">{selectedCocktail.method || 'Build in tin'}</span>
                        </div>
                    </div>
                                        ...
                                        <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-5 py-3 text-right font-mono font-bold text-gray-900 border-l border-gray-100 bg-[#f54900]/[0.02]">
                                                {calculated.unitType === 'liquid'
                                                    ? `${formatMLValue(calculated.ml)} ml`
                                                    : calculated.unitType === 'count'
                                                        ? `${formatNumber(calculated.ml, 0)} ${calculated.originalUnit || ''}`
                                                        : '-'
                                                }
                                                {calculated.unitType === 'liquid' && (
                                                    <div className="text-[10px] text-gray-400 font-normal">
                                                        {formatNumber(calculated.ml / LITER_TO_ML, 2)} L
                                                    </div>
                                                )}
                                            </td>
                                        </tr>
                    ...
                    <div>
                        ...
                        <div className="bg-gray-50 rounded-xl p-5 text-gray-700 leading-relaxed border border-gray-200 text-sm whitespace-pre-line shadow-inner">
                            {selectedCocktail.instructions || "No specific instructions provided for this recipe."}
                        </div>
                    </div>
                </div>
